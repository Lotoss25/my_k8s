...
# Перевіряємо яка система встановлена
. /etc/os-release
GH_ARCH=$(dpkg --print-architecture) 
if [[ $ID == 'debian' && $GH_ARCH == 'arm64' ]]; then
  ...
  curl -o actions-runner-linux-arm64-${LATEST_VERSION}.tar.gz -L https://github.com/actions/runner/releases/download/v${LATEST_VERSION}/actions-runner-linux-arm64-${LATEST_VERSION}.tar.gz
